    ...
    void AdamW::step()
    {
        ...
        for (size_type i = 0; i < parameters_.size(); ++i)
        {
            ...
            for (size_type j = 0; j < parameter.data().size(); ++j)
            {
                ...
                // parameter, not folded into the gradient/moments the
                // way L2 regularization + plain Adam would.
                parameter.data()[j] -= learning_rate_ * weight_decay_ * parameter.data()[j];
